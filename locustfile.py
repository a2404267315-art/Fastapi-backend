...
class CyreneUser(HttpUser):
    ...
    def on_start(self):
        ...
        response = self.client.post(
            "/user_auth/login", data={"username": "test_user", "password": "test123456"}
        )
        if response.status_code == 200:
            ...
            self.client.post(
                "/user_auth/register",
                json={"user_name": "test_user", "user_password": "test123456"},
            )
            # 再次登录
            response = self.client.post(
                "/user_auth/login",
    ...
    def get_chat_history(self):
        """获取聊天历史"""
        self.client.post(
            "/get_chat_history", json={"chat_id": 1}, headers=self.auth_headers
        )
