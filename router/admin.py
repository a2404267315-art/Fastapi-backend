...
def list_all_user(
    ...
    user_management=UserManagement(db)
    result_table=user_management.show_user(page_size=body.page_size,page_number=body.page_number)
...
def list_all_users_conversation(
    ...
    result_table=conversation_management.show_all_conversation(page_size=body.page_size,page_number=body.page_number)
    data_list = [row._mapping for row in result_table]
    return data_list
...
def create_character(
    ...
    character=character_management.create_character(body.character_name,body.system_prompt)
    if not character:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="角色创建失败！"
