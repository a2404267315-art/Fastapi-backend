...
def new_conversation(
    ...
    new_chat=conversation_management.create_conversation(current_user.user_id,chat_name)
    return {
        "msg":"创建成功！",
...
async def list_all_character(
    ...
    result_table=charater_management.get_character(page_size=body.page_size,page_number=body.page_number)
...
async def list_current_conversation(
    ...
    conversation_management=ConversationManagement(db)
    result_table=conversation_management.show_user_conversation(user_id,page_size=body.page_size,page_number=body.page_number)
    data_list = [row._mapping for row in result_table]
    return [data_list]
