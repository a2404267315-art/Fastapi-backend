...
def register_user(
    request: Request, user_data: UserRegisterRequest, db: Session = Depends(get_db)
):
    # 验证图形验证码
    ...
    try:
        success = user_mananger.create_user(name=user_data.user_name, password=password)
        if success:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="注册服务出错了！"
        )
...
def login(
    ...
    if (
        not user
        or user.is_deleted
        or not SecurityUtils.verify_password(
    ...
    access_token = SecurityUtils.create_access_token(
        data={"sub": user.name, "user_id": user.user_id},
    )
    return {
